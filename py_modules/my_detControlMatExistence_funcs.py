...
#-------------------------------------------------------------------------------------------------------------
def eval_Fmat(parmObj,CLmat,candFset,domeig_mags): # items we populate for each F tried
...
def detControlMatExistence(parmObj, A, B, indicMat,indicMat_table,act_locs,perf_locs):
...
# Compute good (Fp,Fq) sample space
    ...
    print('ex=',[act_locs[int(0)], perf_locs[int(0)]])
    for idx in f_ub_table[:,0]: # pull APNP idx from first col of f_ub_table
        str_arr=np.append(str_arr,[np.array([act_locs[int(idx)], perf_locs[int(idx)]])],axis=0) # adds 2x1 col for each APNP pair
...
# Initialize arrays, will be populated in loops
    ...
    numsamp=225 # try 225 different Fs
